...
public class BedRoomTab extends Fragment {
    View view;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        toggle01.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked) {
                    ...
                    String url="http://IP/index.php?ip=192.168.0.218&status=on";
                    ...
                    OkHttpClient client=new OkHttpClient();
                    String url="http://IP/index.php?ip=192.168.0.218&status=off";
                    Request request=new Request.Builder().url(url).build();
                    client.newCall(request).enqueue(new Callback() {
                        @Override
                        public void onFailure(@NotNull Call call, @NotNull IOException e) {
        ...
        toggle02.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked) {
                    OkHttpClient client=new OkHttpClient();
                    String url="http://IP/index.php?ip=192.168.0.142&status=on";
                    Request request=new Request.Builder().url(url).build();
                    client.newCall(request).enqueue(new Callback() {
                        @Override
                        public void onFailure(@NotNull Call call, @NotNull IOException e) {
                    ...
                    OkHttpClient client=new OkHttpClient();
                    String url="http://IP/index.php?ip=192.168.0.142&status=off";
                    Request request=new Request.Builder().url(url).build();
                    client.newCall(request).enqueue(new Callback() {
                        @Override
                        public void onFailure(@NotNull Call call, @NotNull IOException e) {
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    getxmldata("192.168.0.218","192.168.0.142");
                } catch (Exception e) {
