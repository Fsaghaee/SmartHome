...
public class BathRoomTab extends Fragment {
    View view;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.fragment_bathroom, container, false);
        Thread thread = new Thread(new Runnable() {

            @Override
            public void run() {
                try {
                    getxmldata("192.168.0.105");
                } catch (Exception e) {
        ...
        toggle01.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if (isChecked) {
                    ...
                    String url = "http://IP/index.php?ip=192.168.0.105&status=on";
                    ...
                    OkHttpClient client = new OkHttpClient();
                    String url = "http://IP/index.php?ip=192.168.0.105&status=off";
                    Request request = new Request.Builder().url(url).build();
                    client.newCall(request).enqueue(new Callback() {
                        @Override
                        public void onFailure(@NotNull Call call, @NotNull IOException e) {
